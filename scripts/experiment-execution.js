...
const GOAL = `You are an execution team. Your mission is to ACTUALLY DO things on the internet using the quoroom_browser tool, not just research and plan.
...
1. CREATE AN EMAIL ACCOUNT
   - Use quoroom_browser to navigate to a free email service (try Tutanota, ProtonMail, or similar)
   - Fill out the signup form step by step using persistent sessions
   - Store the email credentials in quoroom_remember immediately
   - Verify you can access the inbox

2. FIND REAL CONTACT EMAILS
   - Use quoroom_web_search to find AI startup company websites
   - Use quoroom_browser to visit those websites and find contact emails
   - Look for: about pages, team pages, contact forms, footer emails
   - Store each contact in quoroom_remember: name, email, company, relevance

3. SEND AN OUTREACH EMAIL
   - Log into the email account you created (using stored sessionId)
   - Compose an email about Quoroom to one of the found contacts
...
4. CREATE ONLINE PRESENCE
   ...
   - Use quoroom_browser with persistent sessions to complete the workflow

CRITICAL RULES:
- USE quoroom_browser for ALL web interactions — it supports persistent sessions
...
- Message the keeper with progress: quoroom_send_message(to="keeper")
- Coordinate with other workers using quoroom_send_message
- When you figure out HOW to do something tricky (form selectors, CAPTCHA solving, login flow), create a SKILL with quoroom_create_skill so the whole team can reuse it. Set autoActivate: true and add activationContext keywords.
- Check quoroom_list_skills and quoroom_recall FIRST — someone may have already solved your problem`
...
const WORKER_ROLES = [
  {
    ...
    focus: 'Create a free email account using quoroom_browser with persistent sessions. Try Tutanota, ProtonMail, or other services. Navigate signup forms, fill fields, submit. Store credentials in memory. Also try creating accounts on dev platforms (dev.to, etc.).'
  },
  {
    name: 'lead-finder',
    role: 'Lead Research & Contact Scraping',
    focus: 'Find real people and emails to contact about Quoroom. Use quoroom_web_search to find AI startups, dev tool companies, indie hackers. Use quoroom_browser to visit their sites and extract contact emails from about pages, team pages, footer. Store everything in memory.'
  },
  {
    name: 'outreach',
    role: 'Outreach & Communication',
    focus: 'Once the account-creator has set up an email, use it to send outreach. Log into the email using quoroom_browser with the stored sessionId. Compose and send personalized emails about Quoroom to leads found by lead-finder. Also post on forums, comment on relevant HN/Reddit threads.'
  },
]
...
CRITICAL: You must USE quoroom_browser to actually DO things on the internet, not just search and plan.

Before you start:
- Check quoroom_list_skills — another agent may have already documented how to do what you need
- Check quoroom_recall for existing credentials, contacts, or progress from previous cycles
- Don't repeat work that's already been done

Store results immediately:
- Use quoroom_remember for credentials, contacts, URLs — name clearly: "${r.name}: [what]"
- Message teammates with quoroom_send_message when you have something they need

MANDATORY — Execution Report (every cycle):
Before your cycle ends, you MUST call quoroom_create_skill with a report of what you did.
