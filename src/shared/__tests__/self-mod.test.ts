...
describe('revertModification', () => {
  ...
  it('restores skill content when snapshot exists', () => {
    const skill = queries.createSkill(db, roomId, 'Test Skill', 'old content', { autoActivate: true })
    const entry = performModification(
      db,
      roomId,
      workerId,
      `/skills/${skill.id}`,
    ...
    queries.saveSelfModSnapshot(db, entry.id, 'skill', skill.id, 'old content', 'new content')
    queries.updateSkill(db, skill.id, { content: 'new content', version: skill.version + 1 })

    revertModification(db, entry.id)

    const revertedSkill = queries.getSkill(db, skill.id)
    expect(revertedSkill?.content).toBe('old content')
    const history = getModificationHistory(db, roomId)
    expect(history.find(h => h.id === entry.id)?.reverted).toBe(true)
  })
})
