...
export function runMigrations(database: Database.Database, log: (msg: string) => void = console.log): void {
  ...
  // Migration 25: per-room autonomy, worker model, and queen safeguard columns
  applyMigration(database, 25, (db) => {
    ...
    // Seed autonomy_mode and max_concurrent_tasks from global settings if present
    ...
    if (autoMode?.value) db.prepare('UPDATE rooms SET autonomy_mode = ?').run(autoMode.value)
